...
protected: 
    ...
    Piece(Color color, std::string_view symbol, char type, int val) :
        PieceColor(color), symbol(std::move(symbol)), pieceType(type), value(val) {}
...
public:
    ...
    if (TargetPiece != nullptr && TargetPiece->getColor() == this->PieceColor) return false;
